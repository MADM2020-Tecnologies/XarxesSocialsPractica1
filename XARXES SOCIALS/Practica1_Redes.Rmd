...
#Pruebas con los MACROdatos.

```{r}

bigraph = graph.data.frame(data, directed = F)
plot(bigraph)
...
degreesbig <- degree(graph = bigraph)
degreesbig
...
degrees1big <- which(degrees == 1)
vertexNamesbig <- names(degrees1big)
vertexNamesbig
...
neighbors(graph = graph, v = vertexNamesbig)
...
```{r}
vertexConnectivityBoolbig <- vertex_connectivity(bigraph, source = NULL, target = NULL, checks = TRUE)
vertexConnectivityBoolbig
```
...
```{r, echo=FALSE}
if(vertexConnectivityBoolbig > 0) {
  print("Graph is connected")
} else {
  print("Graph is not connected")
}
...
# b) Compute the edge density
```{r}
densitybig <- edge_density(bigraph, loops = F)
```
```{r, echo = FALSE}
print(paste("The edge density is", densitybig, sep = " "))
```
...
```{r}
meanDistancebig <- mean_distance(bigraph, directed=F)
```
```{r, echo = FALSE}
print(paste("The mean distance is", meanDistancebig, "nodes", sep = " "))
```
...
vertex1big <- '1000'
vertex2big <- '2000'
paintedGrafbig <- bigraph
shortestPathbig <- get.shortest.paths(
        bigraph,
        from = V(bigraph)$name == vertex1big,
        to = V(bigraph)$name == vertex2big,
        weights = NULL,
        output = "vpath",
        predecessors = FALSE,
        inbound.edges = FALSE
...
E(paintedGrafbig, path=shortestPathbig$vpath[[1]])$color <- "red"
plot(paintedGrafbig)
```
...
```{r}
AddWeightsFromDegreebig <- function (bigraph) {
  res <- bigraph
  for(v in V(graph = res)$name) {
    degree <- degree(res, v)
    #print(paste('vertex', v, 'have a degree if', degree, sep=' '))
...
is_weighted(bigraph)
weightedGraphbig <- AddWeightsFromDegreebig(bigraph)
is_weighted(weightedGraphbig)
E(weightedGraphbig)[from(V(weightedGraphbig)$name[1])]$weight
```

Calculate the diameter

```{r}
diameterbig <- diameter(weightedGraphbig, directed = FALSE)
diameterNodesbig <- get_diameter(weightedGraphbig, directed = FALSE)
```
```{r, echo = FALSE}
print(paste("The graph diameter is", diameterbig, "units", sep = " "))
```
